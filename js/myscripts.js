...
function filterFunction(id) {
  ...
  if (btnReq.classList.contains("active")) {
    for (i = 0; i < x.length; i++) {
      if (x[i].className.indexOf(btnReq.id) > -1) {
        w3AddClass(x[i], "show");
      } else {
        w3RemoveClass(x[i], "show");
      }
  ...
  if (btnVP.classList.contains("active")) {
    for (i = 0; i < x.length; i++) {
      if (x[i].querySelectorAll(".points").length > 0) {
        w3AddClass(x[i], "show");
      } else {
        w3RemoveClass(x[i], "show");
      }
  ...
  if (priceValue > 0) {
    for (i = 0; i < x.length; i++) {
      if (x[i].querySelector(".price") == null) {
        cardValue = 0;
      } else {
        cardValue = parseInt(x[i].querySelector(".price").textContent);
      }
      if (cardValue >= priceValue) {
        w3AddClass(x[i], "show");
      } else {
        w3RemoveClass(x[i], "show");
      }
  ...
  if (document.getElementById("reqs").classList.contains("active")) {
    document
      .getElementById("subfilterReqs")
      .classList.remove("subfilterReqs-disabled"); //enabling the subfilters
    ...
    if (
      ...
      for (i = 0; i < x.length; i++) {
        ...
        if (temperatureValue > -30) {
          if (temperatureValue <= temperatureData) {
            show = true;
          }
        }
        if (oxygenValue > 0 && oxygenData > 0) {
          if (oxygenValue <= oxygenData) {
            show = true;
          }
        }
        if (oceansValue > 0) {
          if (oceansValue <= oceansData) {
            show = true;
          }
        }
        if (venusValue > 0) {
...
function clearInput() {
  ...
  document.getElementById("buttonsContainer-body").style.height =
    CONTAINER + "px";
  document.getElementById("contentFilters").style.height =
    CONTENT_FILTERS + "px";
  ...
  for (i = 0; i < x.length; i++) {
    w3RemoveClass(x[i], "clicked-card");
  }
  selectedCards = "";
...
function toggleRequirementsFilters() {
  ...
  if ($("#subfilterReqs:visible").length == 0) {
    ...
    document.getElementById("buttonsContainer-body").style.height =
      containerHeight + "px";
    document.getElementById("contentFilters").style.height =
      contentFiltersCurrent + "px";
    setTimeout(function () {
      $("#subfilterReqs").fadeIn(200);
    }, 100);
  } else {
    containerHeight = containerHeight - 105;
    contentFiltersCurrent = contentFiltersCurrent - 105;
  ...
  if ($("#buttonsContainer:visible").length > 0) {
    ...
    if (selectedCards.length > 0) {
      document.getElementById("btn-selectedCards").href =
        "https://ssimeonoff.github.io/cards-list" + selectedCards;
